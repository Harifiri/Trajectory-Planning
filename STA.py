...
RightTurnPoint = [[(-Rw-2*Lane, -5*Rw/6), (-Rw, -5*Rw/6), (-Rw, -Rw), (-5*Rw/6, -Rw), (-5*Rw/6, -Rw-2*Lane)],
                 [(5*Rw/6, -Rw-2*Lane), (5*Rw/6, -Rw), (Rw, -Rw), (Rw, -5*Rw/6), (Rw+2*Lane, -5*Rw/6)],
                 [(Rw+2*Lane, 5*Rw/6), (Rw, 5*Rw/6), (Rw, Rw), (5*Rw/6, Rw), (5*Rw/6, Rw+2*Lane)],
                 [(-5*Rw/6, Rw+2*Lane), (-5*Rw/6, Rw), (-Rw, Rw), (-Rw, 5*Rw/6), (-Rw-2*Lane, 5*Rw/6)]]

def GetSTAPoint():
    for value in np.arange(-Rw-2*Lane,Rw+2*Lane,v*dt):
        if abs(value-Rw-2*Lane)<2:
            break
...
if __name__=="__main__":
    ...
    print(len(STA_point_[0]))
    print(len(STA_point_[1]))
    print(len(STA_point_[2]))
